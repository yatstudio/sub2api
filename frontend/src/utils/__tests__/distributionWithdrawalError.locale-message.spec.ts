...
describe('distributionWithdrawalError locale message alignment', () => {
  it('returns readable zh messages for cooldown/daily count/daily amount limits', () => {
    ...
    expect(resolveDistributionWithdrawalErrorMessage({
      error: { code: 'distribution_withdrawal_cooldown' }
    }, t)).toBe('当前处于提现冷却期，请稍后再试')
    expect(resolveDistributionWithdrawalErrorMessage({
    ...
    expect(resolveDistributionWithdrawalErrorMessage({
      error: { code: 'DISTRIBUTION_WITHDRAWAL_DAILY_AMOUNT_LIMIT' }
    }, t)).toBe('今日提现金额已达上限')
    expect(resolveDistributionWithdrawalErrorMessage({
    ...
    expect(resolveDistributionWithdrawalErrorMessage({
      error: { reason: 'DISTRIBUTION_WITHDRAWAL_DAILY_AMOUNT_LIMIT' }
    }, t)).toBe('今日提现金额已达上限')
    expect(resolveDistributionWithdrawalErrorMessage({
    ...
    expect(resolveDistributionWithdrawalErrorMessage({
      error: 'DISTRIBUTION_WITHDRAWAL_DAILY_AMOUNT_LIMIT'
    }, t)).toBe('今日提现金额已达上限')
    expect(resolveDistributionWithdrawalErrorMessage({
      data: { error: 'DISTRIBUTION_WITHDRAWAL_DAILY_LIMIT' }
    }, t)).toBe('今日提现次数已达上限')
    ...
    expect(resolveDistributionWithdrawalErrorMessage({
      error: { message: 'request blocked: distribution_withdrawal_cooldown' }
    }, t)).toBe('当前处于提现冷却期，请稍后再试')
    expect(resolveDistributionWithdrawalErrorMessage({
